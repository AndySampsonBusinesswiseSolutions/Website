...
namespace GetMeterSpecificProfile.api.Controllers
{
    ...
    public class GetMeterSpecificProfileController : ControllerBase
    {
        ...
        public GetMeterSpecificProfileController(ILogger<GetMeterSpecificProfileController> logger, IConfiguration configuration)
        {
            ...
            getMeterSpecificProfileAPIId = new Methods.SystemSchema.API().API_GetAPIIdByAPIGUID(new Enums.SystemSchema.API.GUID().GetMeterSpecificProfileAPI);
        }
        ...
        {
            //Launch API process
            new Methods.SystemSchema.API().PostAsJsonAsync(getMeterSpecificProfileAPIId, hostEnvironment, JObject.Parse(data.ToString()));
