...
namespace ValidateUsageUploadTempMeterUsageData.api.Controllers
{
    [EnableCors]
    [ApiController]
    public class ValidateUsageUploadTempMeterUsageDataController : ControllerBase
    {
        private readonly ILogger<ValidateUsageUploadTempMeterUsageDataController> _logger;
        ...
        private readonly Int64 validateUsageUploadTempMeterUsageDataAPIId;

        public ValidateUsageUploadTempMeterUsageDataController(ILogger<ValidateUsageUploadTempMeterUsageDataController> logger)
        {
            _logger = logger;
            _methods.InitialiseDatabaseInteraction(_systemAPINameEnums.ValidateUsageUploadTempMeterUsageDataAPI, _systemAPIPasswordEnums.ValidateUsageUploadTempMeterUsageDataAPI);
            validateUsageUploadTempMeterUsageDataAPIId = _systemMethods.API_GetAPIIdByAPIGUID(_systemAPIGUIDEnums.ValidateUsageUploadTempMeterUsageDataAPI);
        }

        [HttpPost]
        [Route("ValidateUsageUploadTempMeterUsageData/IsRunning")]
        public bool IsRunning([FromBody] object data)
        {
            ...
            //Launch API process
            _systemMethods.PostAsJsonAsync(validateUsageUploadTempMeterUsageDataAPIId, callingGUID, jsonObject);

            return true;
        }

        [HttpPost]
        [Route("ValidateUsageUploadTempMeterUsageData/Validate")]
        public void Validate([FromBody] object data)
        {
            ...
            try
            {
                ...
                _systemMethods.ProcessQueue_Insert(
                    ...
                    createdByUserId,
                    sourceId,
                    validateUsageUploadTempMeterUsageDataAPIId);

                //Get CheckPrerequisiteAPI API Id
                var checkPrerequisiteAPIAPIId = _systemMethods.GetCheckPrerequisiteAPIAPIId();

                //Call CheckPrerequisiteAPI API
                var API = _systemMethods.PostAsJsonAsync(checkPrerequisiteAPIAPIId, _systemAPIGUIDEnums.ValidateUsageUploadTempMeterUsageDataAPI, jsonObject);
                ...
                if(erroredPrerequisiteAPIs.Any())
                {
                    ...
                    _systemMethods.ProcessQueue_Update(processQueueGUID, validateUsageUploadTempMeterUsageDataAPIId, true, $" Prerequisite APIs {string.Join(",", erroredPrerequisiteAPIs)} errored");
                    return;
                }
            ...
            catch(Exception error)
            {
                ...
                //Update Process Queue
                _systemMethods.ProcessQueue_Update(processQueueGUID, validateUsageUploadTempMeterUsageDataAPIId, true, $"System Error Id {errorId}");
            }
        }
    }
}
