...
namespace CommitBasketData.api.Controllers
{
    ...
    public class CommitBasketDataController : ControllerBase
    {
        ...
        public CommitBasketDataController(ILogger<CommitBasketDataController> logger, IConfiguration configuration)
        {
            ...
            commitBasketDataAPIId = new Methods.System.API().API_GetAPIIdByAPIGUID(new Enums.SystemSchema.API.GUID().CommitBasketDataAPI);
        }
        ...
        {
            //Launch API process
            new Methods.System.API().PostAsJsonAsyncAndDoNotAwaitResult(commitBasketDataAPIId, hostEnvironment, JObject.Parse(data.ToString()));
        ...
        public void Commit([FromBody] object data)
        {
            ...
            try
            {
                ...
                if(!new Methods.System.API().PrerequisiteAPIsAreSuccessful(new Enums.SystemSchema.API.GUID().CommitBasketDataAPI, commitBasketDataAPIId, hostEnvironment, jsonObject))
                {
                    return;
                }
