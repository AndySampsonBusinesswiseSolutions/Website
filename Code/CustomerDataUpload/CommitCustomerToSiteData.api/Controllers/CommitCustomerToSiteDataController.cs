...
namespace CommitCustomerToSiteData.api.Controllers
{
    ...
    public class CommitCustomerToSiteDataController : ControllerBase
    {
        ...
        public CommitCustomerToSiteDataController(ILogger<CommitCustomerToSiteDataController> logger, IConfiguration configuration)
        {
            ...
            commitCustomerToSiteDataAPIId = new Methods.System.API().API_GetAPIIdByAPIGUID(new Enums.SystemSchema.API.GUID().CommitCustomerToSiteDataAPI);
        }
        ...
        {
            //Launch API process
            new Methods.System.API().PostAsJsonAsyncAndDoNotAwaitResult(commitCustomerToSiteDataAPIId, hostEnvironment, JObject.Parse(data.ToString()));
        ...
        public void Commit([FromBody] object data)
        {
            ...
            try
            {
                ...
                if(!new Methods.System.API().PrerequisiteAPIsAreSuccessful(new Enums.SystemSchema.API.GUID().CommitCustomerToSiteDataAPI, commitCustomerToSiteDataAPIId, hostEnvironment, jsonObject))
                {
                    return;
                }
