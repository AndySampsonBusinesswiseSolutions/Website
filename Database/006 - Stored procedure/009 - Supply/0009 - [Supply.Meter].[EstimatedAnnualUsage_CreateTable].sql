...
GO
IF NOT EXISTS(SELECT TOP 1 1 FROM sys.objects WHERE type = 'P' AND OBJECT_ID = OBJECT_ID('[Supply.Meter].[EstimatedAnnualUsage_CreateTable]'))
    BEGIN
        EXEC('CREATE PROCEDURE [Supply.Meter].[EstimatedAnnualUsage_CreateTable] AS BEGIN SET NOCOUNT ON; END')
    END
GO
...
ALTER PROCEDURE [Supply.Meter].[EstimatedAnnualUsage_CreateTable]
    @MeterId BIGINT
AS
BEGIN
    -- =============================================
