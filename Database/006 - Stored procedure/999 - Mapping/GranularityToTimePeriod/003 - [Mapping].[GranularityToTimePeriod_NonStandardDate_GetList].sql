...
IF NOT EXISTS(SELECT TOP 1 1 FROM sys.objects WHERE type = 'P' AND OBJECT_ID = OBJECT_ID('[Mapping].[GranularityToTimePeriod_NonStandardDate_GetList]'))
    BEGIN
        EXEC('CREATE PROCEDURE [Mapping].[GranularityToTimePeriod_NonStandardDate_GetList] AS BEGIN SET NOCOUNT ON; END')
    END
GO
...
ALTER PROCEDURE [Mapping].[GranularityToTimePeriod_NonStandardDate_GetList]
